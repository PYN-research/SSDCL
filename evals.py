...
from utils import AverageMeter, set_random_seed, normalize
...
def get_auroc(scores_id, scores_ood):
    scores = np.concatenate([scores_id, scores_ood])#(10000,)
...
def compute_ood_score(P, model, ood_score, x, simclr_aug=None):
    ...
    if ood_score == 'clean_norm':
        ...
        feats = get_features(model, simclr_aug, x, layer=P.ood_layer, sample_num=sample_num)#(2,128)
        feats_avg = sum(feats) / len(feats)#(128)
...
def get_features(model, simclr_aug, x, layer='simclr', sample_num=1):
    ...
    for seed in range(sample_num):
        ...
        feats.append(output_aux[layer])#(2,128)
    return feats
