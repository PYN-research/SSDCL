...
def load_checkpoint(logdir, mode='last'):
    if mode == 'last':
        ...
        mu_path = os.path.join(logdir, 'last.parameter')
    elif mode == 'best':
    ...
    if os.path.exists(model_path):
        ...
        means_state = torch.load(mu_path)
        with open(config_path, 'rb') as handle:
            cfg = pickle.load(handle)
    else:
        return None, None, None

    return model_state, optim_state, means_state, cfg
...
def save_checkpoint(epoch, model_state, optim_state,  logdir):
    ...
    #last_means = os.path.join(logdir, 'last.parameter')
    ...
    #torch.save(mu_state, last_means)
