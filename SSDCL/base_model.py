...
class BaseModel(nn.Module, metaclass=ABCMeta):
    def __init__(self, last_dim, num_classes=10, simclr_dim=128):
        ...
        self.shift_cls_layer = nn.Linear(last_dim, 4)#2
        #self.em_layer = nn.Linear(last_dim, 64)   #64  
    ...
    def forward(self, inputs, penultimate=False, simclr=False, shift=False,em_unfolding=False,  joint=False):
        _aux = {}
        _return_aux = False
        ...
        if em_unfolding:
            _return_aux = True
            _aux['em_unfolding'] = self.em_layer(features)
