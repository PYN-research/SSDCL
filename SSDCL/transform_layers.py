...
class RandomResizedCropLayer(nn.Module):
    ...
    def _sample_latent(self, inputs):
        ...
        _device = inputs.device
        N, _, width, height = inputs.shape#(128*8,_,32,32)

        # N * 10 trial
        area = width * height
        target_area = np.random.uniform(*self.scale, N * 10) * area#(N*10,)
        log_ratio = (math.log(self.ratio[0]), math.log(self.ratio[1]))
        aspect_ratio = np.exp(np.random.uniform(*log_ratio, N * 10))#(N*10,)

        # If doesn't satisfy ratio condition, then do central crop
        w = np.round(np.sqrt(target_area * aspect_ratio))#(N*10,)
        h = np.round(np.sqrt(target_area / aspect_ratio))#(N*10,)
        ...
        #w_bias = (w - width, width - w + 1)/ width#np.random.randint(
        #h_bias = (h - height, height - h + 1)/ width
        w_bias = np.array([np.random.randint((w - width)[i], (width - w + 1)[i]) for i in range(len(w))]) / width
        h_bias = np.array([np.random.randint((h - height)[i], (height - h + 1)[i]) for i in range(len(h))]) / height
        w = w / width
        h = h / height
