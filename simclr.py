...
def train(P, epoch, model, criterion, optimizer, scheduler, loader, logger=None,
    ...
    for n, (images, labels) in enumerate(loader):
        ...
        transform = get_simclr_augmentation(images_pair)
        images_pair=[transform(images_pair[i].cpu()) for i in range(len(images_pair))]
        images_pair =torch.tensor([item.cpu().detach().numpy() for item in images_pair]).cuda()
